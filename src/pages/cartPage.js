      ...
      ${cart.map((item) => /* html */ `
        <tr>
          <td class="hidden pb-4 md:table-cell">
            <a href="#">
              <img src="${item.img[0]}" class="w-20 rounded" alt="${item.img[0]}">
            </a>
          </td>
          <td>
            <p class="mb-2 md:ml-4">${item.title}</p>
              <button data-id="${item.id}" data-id="${item.id}" class="btn btn-remove text-gray-700 md:ml-4">
                <small>(Remove item)</small>
              </button>
          </td>
          <td class="pt-[40px] justify-center md:justify-between md:flex ">
          <div class="w-20 h-10 flex justify-between">
          <button data-id="${item.id}" class="btn btn-increase rounded-md bg-green-500 p-2">Tăng</button>
            <div class="mx-2 flex w-10 h-10">
                <input type="number" value="${item.quantity}" 
                class="w-10 font-semibold rounded-md text-center text-gray-700 bg-gray-200 outline-none focus:outline-none hover:text-black focus:text-black" />
            </div>
        <button data-id="${item.id}" class="btn btn-decrease rounded-md bg-orange-500 p-2">Giảm</button>
          </div>
          ...
          <p class="mb-6 italic">Shipping and additionnal costs are calculated based on values you have entered</p>
            <div class="flex justify-between border-b">
              ...
              <button class="flex justify-center w-full px-10 py-3 mt-6 font-medium text-white uppercase bg-gray-800 rounded-full shadow item-center hover:bg-gray-700 focus:shadow-outline focus:outline-none" id="btn-pay">
                ...
                <span class="ml-2 mt-5px">Đặt hàng</span>
              </button>
        </div>
      </div>
    </div>
